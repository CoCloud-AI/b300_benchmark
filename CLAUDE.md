...
- **Cycle-start protocol (added 2026-07-17, after the EP blind spot).** Before locking a new cycle's sweep plan: (1) re-verify every Known Issues entry marked STALE or older than one cycle with a ~1 h capability smoke matrix (at minimum: EP × each framework × one model) — an April conclusion must not silently govern a July cycle; (2) review our open upstream issues/PRs (sgl-project/sglang#24502 #31503 #31556, vllm-project/vllm#48921, plus threads we commented on) for maintainer replies with actionable workarounds — the working cutedsl+DeepEP-LL answer sat in our own issue thread for 9 days while the 610 cycle ran TP-only; (3) every new Known Issues entry MUST carry "as of <framework versions, date>" and is treated as expired after one cycle unless re-verified.
...
- EP: **broken on both frameworks for NVFP4 — as of the April 2026 stack (vLLM 0.13.0 / SGLang 0.5.10.post1). STALE: re-verify at next cycle start.** Upstream has since shipped EP paths: SGLang registered flashinfer_cutedsl + DeepEP low-latency (sgl-project/sglang#25525; recommended by maintainers in our #24502 on 2026-06-29), and vLLM EP works with ModelOpt NVFP4 on its 2026 releases — independent B300 results run TP4+EP4 / TP2+EP2 with large per-GPU gains vs TP8 for small-active-param MoE. Original April failures for the record: vLLM 0.13.0 rejected EP at kernel dispatch (`cutlass_moe_fp4` won't accept `expert_map`); SGLang 0.5.10.post1 crashed during model load in `modelopt_quant.py:1754` with a w13_input_scale [256] vs w13_weight_scale_2 [32] shape mismatch (verified on M2.7 NVFP4 2026-04-14; our fix PR: sgl-project/sglang#31503). Next cycle: EP/replica configuration matrix is the top-priority smoke test.
...
## Known Issues
- **EP+NVFP4 broken on BOTH vLLM 0.13.0 AND SGLang 0.5.10.post1 — as of April 2026; STALE, superseded upstream (see the EP entry under Benchmark Config).** vLLM rejected at the kernel dispatcher; SGLang crashed during ModelOpt weight post-processing with a w13_input_scale [256] vs w13_weight_scale_2 [32] shape mismatch (verified on M2.7 2026-04-14 00:52; our fix PR sgl-project/sglang#31503, pin-bump PR #31556). Do not carry "TP-only" forward as a rule — re-verify EP at every cycle start.
